...
def plan(request):
	return render(request, 'excelapp/plan.html')

def otherpayment(request):
    return render(request, 'excelapp/otherpayment.html')


def about(request):
	return render(request, 'excelapp/company.html')


def career(request):
	return render(request, 'excelapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('excelapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'excelapp/login.html')


def privacy(request):
	return render(request, 'excelapp/privacy.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('excelapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('excelapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
		...
		second_template= render_to_string('excelapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@AgroMineHub.com'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'excelapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'excelapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'excelapp/adminpage.html', context)


def terms(request):
	return render(request, 'excelapp/terms.html')


def irrigation_system(request):
	return render(request, 'excelapp/irrigation_system.html')


def seeds(request):
	return render(request, 'excelapp/seeds.html')


def livestocks(request):
	return render(request, 'excelapp/livestocks.html')


def rawresources(request):
	return render(request, 'excelapp/rawresources.html')


def contact(request):
	return HttpResponse('Contact us using support@agrominehub.com')
...
def agriculture(request):
	return render(request, 'excelapp/agriculture.html')
...
def investmentPlans(request):
	return render(request, 'excelapp/investmentplans.html')

@login_required(login_url='signin')
def irrigationDeposit(request):
	form = IrrigationDepositForm(request.POST or None)
	if form.is_valid():
	...
	context= {'form':form}
	return render(request, 'excelapp/retirementdepositForm.html', context)
...
def seedsDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'excelapp/seedsDepositForm.html', context)
...
def livestockDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'excelapp/livestockDepositForm.html', context)
...
def rawresourceDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'excelapp/RawresourceDepositForm.html', context)
...
def portfolioDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'excelapp/PortfolioDepositForm.html', context)
...
def deposit(request):
	...
	if price_amount and pay_currency:
		...
		payload=json.dumps({
			...
			"success_url": "https://www.AgroMineHub.com/dashboard",
			#our success url will direct us to the get_payment_status view for balance top ups
			"cancel_url": "https://www.AgroMineHub.com/dashboard"
		})
		...
		template= render_to_string('excelapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
		plain_message= strip_tags(template)
		...
		emailmessage.send()
		try:
			send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@AgroMineHub.com'])
		except BadHeaderError:
			pass
		return redirect(generated_link)
		context={}
	return render(request, 'excelapp/deposittwo.html', context)
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@AgroMineHub.com'])
	...
	context={}
	return render(request, 'excelapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'excelapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'excelapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'excelapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'excelapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'excelapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'excelapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'excelapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('excelapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('excelapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'excelapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('excelapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'excelapp/account_settings.html', context)
...
def notifications(request):
	...
	return render(request, 'excelapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('excelapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('excelapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@AgroMineHub.com'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'excelapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'excelapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('excelapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('excelapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@AgroMineHub.com'])
			except BadHeaderError:
				pass
	...
	return render(request, 'excelapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@AgroMineHub.com'])
	...
	return render(request, 'excelapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'excelapp/portfolio.html', context)